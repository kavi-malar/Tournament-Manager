...
exports.createTournament = async (req, res) => {
  try {
    const { name, description, sport, format, status, max_teams, prize_pool, start_date, end_date, registration_deadline } = req.body;
    if (!name || !sport || !format || !max_teams || !start_date || !end_date)
      return res.status(400).json({ success: false, message: 'Required fields missing' });

    const [result] = await db.query(
      `INSERT INTO tournaments (name, description, sport, format, status, max_teams, prize_pool, start_date, end_date, organizer_id, registration_deadline)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description || '', sport, format, status || 'upcoming', max_teams, prize_pool || '', start_date, end_date, req.user.id, registration_deadline || null]
    );

    try {
      await db.query(
        `INSERT INTO audit_log (user_id, action, entity_type, entity_id, description) VALUES (?, 'CREATE', 'tournament', ?, ?)`,
        [req.user.id, result.insertId, 'Created tournament: ' + name]
      );
    } catch (_) {}
...
exports.updateTournament = async (req, res) => {
  try {
    ...
    const isOrganizer = parseInt(tournament.organizer_id) === parseInt(req.user.id);
    if (!isAdmin && !isOrganizer) {
      return res.status(403).json({ success: false, message: 'Not authorized to edit this tournament' });
    }

    await db.query(
      `UPDATE tournaments SET name=?, description=?, sport=?, format=?, status=?, max_teams=?, prize_pool=?, start_date=?, end_date=?, registration_deadline=? WHERE id=?`,
      [
    ...
    try {
      await db.query(
        `INSERT INTO audit_log (user_id, action, entity_type, entity_id, description) VALUES (?, 'UPDATE', 'tournament', ?, ?)`,
        [req.user.id, req.params.id, 'Updated tournament: ' + (name || tournament.name)]
      );
    } catch (_) {}
...
// BUG FIX: Allow admin OR the organizer who owns it to delete
exports.deleteTournament = async (req, res) => {
  try {
    ...
    try {
      await db.query(
        `INSERT INTO audit_log (user_id, action, entity_type, entity_id, description) VALUES (?, 'DELETE', 'tournament', ?, ?)`,
        [req.user.id, req.params.id, 'Deleted tournament: ' + tournament.name]
      );
    } catch (_) {}
...
exports.registerTeam = async (req, res) => {
  try {
    ...
    if (tournament[0].registration_deadline) {
      const now = new Date();
      const deadline = new Date(tournament[0].registration_deadline);
      if (now > deadline)
    ...
    // Notify team members of successful registration
    try {
      await db.query(
        `INSERT INTO notifications (user_id, type, title, message, entity_type, entity_id)
         SELECT tm.user_id, 'registration', 'Tournament Registration',
           CONCAT('Your team has been registered for: ', ?),
           'tournament', ?
         FROM team_members tm WHERE tm.team_id = ?`,
        [tournament[0].name, req.params.id, team_id]
      );
    } catch (_) {}

    try {
      await db.query(
        `INSERT INTO audit_log (user_id, action, entity_type, entity_id, description) VALUES (?, 'REGISTER', 'tournament', ?, ?)`,
        [req.user.id, req.params.id, 'Registered team ' + team_id]
      );
    } catch (_) {}
